...
std::string CTSLanguage::SymbolName(TSSymbol symbol)  const {
	return m_language ? std::string(ts_language_symbol_name(m_language, symbol)) : "ERROR: No Language Initialized!";
}


TSSymbol CTSLanguage::SymbolForName(const std::string &name, bool is_named) const {
	return m_language ? ts_language_symbol_for_name(m_language, name.c_str(), name.length(), is_named) : 0;
}
...
inline std::string CTSLanguage::FieldNameForId(TSFieldId id)  const {
	return m_language ? std::string(ts_language_field_name_for_id(m_language, id)) : "Error: No Language Initialized!";
}
