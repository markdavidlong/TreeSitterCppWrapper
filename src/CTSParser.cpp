...
std::shared_ptr<CTSTree> CTSParser::Parse(const std::shared_ptr<CTSTree>& old_tree, TSInput input) const
{
	return std::make_shared<CTSTree>(ts_parser_parse(m_self, /*old_tree->GetEncapsulatedTSTree()*/ nullptr, input));
}
...
{
	return std::make_shared<CTSTree>(ts_parser_parse_string(m_self, nullptr, str.c_str(), str.length()));
}

std::shared_ptr<CTSTree> CTSParser::ParseString(const std::shared_ptr<CTSTree>& old_tree, const std::string& str) const
{
	return make_shared<CTSTree>(ts_parser_parse_string(m_self, old_tree->m_tree, str.c_str(), str.length()));
}

#ifdef HANDLE_UTF
TSTree* CTSParser::ParseStringEncoding(const TSTree* old_tree, const char* str, uint32_t length, TSInputEncoding encoding)
{
